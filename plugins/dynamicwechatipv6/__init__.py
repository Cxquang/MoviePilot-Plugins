...
from app.plugins.dynamicwechatipv6.helper import PyCookieCloud, MySender, IpLocationParser


class DynamicWeChatIpv6(_PluginBase):
    # 插件名称
    plugin_name = "动态企微可信IP--支持ipv6"
    # 插件描述
    ...
    # 作者主页
    author_url = "https://github.com/Cxquang/MoviePilot-Plugins"
    # 插件配置项ID前缀
    plugin_config_prefix = "dynamicwechatipv6_"
    # 加载顺序
    plugin_order = 47
    # 可使用的用户级别
    auth_level = 2
    ...
    def forced_change(self, event: Event = None):
        ...
        if event:
            ...
            if not event_data or event_data.get("action") != "dynamicwechatipv6":
                return
        # 先尝试cookie登陆
        try:
